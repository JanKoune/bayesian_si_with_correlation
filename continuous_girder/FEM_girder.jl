...
`fem_general_single_girder` relates to `fem_prismatic_single_girder`.
"""
function fem_general_twin_girder_torsion(
    node_xs_g::AbstractArray{Float64,1},
    Ly::Float64,
    elem_EI_g::AbstractArray{Float64,1},
    elem_GJ_g::AbstractArray{Float64,1},
    support_positions::AbstractArray{Float64,1};
    support_dofs::AbstractArray{Int32}=zeros(Int, 0, 3),
    vert_spring_positions::AbstractArray{Float64,1}=zeros(0),
    vert_spring_stiffnesses::AbstractArray{Float64,1}=zeros(0),
    tors_spring_positions::AbstractArray{Float64,1}=zeros(0),
    tors_spring_stiffnesses::AbstractArray{Float64,1}=zeros(0),
    left_hinge_positions::AbstractArray{Float64,1}=zeros(0),
    right_hinge_positions::AbstractArray{Float64,1}=zeros(0),
)
    # ................................................
    # Initialize
    # ................................................
    num_node = 2 * length(node_xs_g)
    num_elem = 2 * length(elem_EI_g)
    num_elem_per_girder = length(elem_EI_g)
    num_dof = num_node * 3
    num_support = length(support_positions)

    # Additional nodes for link elements. Each of these
    # nodes has 2 dofs: Vertical translation and rotation
    idx_start_link = Int(10^(ceil(log10(num_node))))
    num_node_link = length(node_xs_g) * 2
    num_link_elem = num_elem_per_girder + 1
    num_dof_link = num_node_link * 3

    # Duplicate element stiffnesses
    elem_EI = interleave_vectors(elem_EI_g, elem_EI_g)
    elem_GJ = interleave_vectors(elem_GJ_g, elem_GJ_g)

    # Get element nodes table
    elem_nodes = hcat([1:num_node - 2;], [3:num_node;])

    # Get nodes of links that connect the twin girders
    link_nodes = hcat([1:2:num_node - 1;], [2:2:num_node;])

    # TODO: to be improved, done twice
    support_dofs = _fem_general_input_check_torsion(
        node_xs_g, elem_EI_g, elem_GJ_g, support_positions, support_dofs
    )

    # Assemble node coordinate matrix
    nodes = zeros(num_node, 3)
    nodes[1:2:end, 1] = node_xs_g
    nodes[1:2:end, 2] .= -Ly/2
    nodes[2:2:end, 1] = node_xs_g
    nodes[2:2:end, 2] .= Ly/2

    function _find_hinges(hinge_positions, node_xs, n_elem)
        hinges = zeros(Int, n_elem)
        if length(hinge_positions) > 0
            hinge_nodes = [
                findfirst(node_xs .≈ hinge_position) for hinge_position in hinge_positions
            ]
    ...
    hinges_left = _find_hinges(left_hinge_positions, node_xs_g, num_elem_per_girder)
    hinges_right = _find_hinges(right_hinge_positions, node_xs_g, num_elem_per_girder)
    hinges = interleave_vectors(hinges_left, hinges_right)

    # Create arrays of vertical and rotational stiffness per link
    Kv = zeros(num_link_elem)
    Kphi = zeros(num_link_elem)

    # Find positions of springs
    vert_spring_idx = []
    for pos in vert_spring_positions
        append!(vert_spring_idx, findfirst(node_xs_g .≈ pos))
    end

    tors_spring_idx = []
    for pos in tors_spring_positions
        append!(tors_spring_idx, findfirst(node_xs_g .≈ pos))
    end

    # Add stiffnesses to arrays
    Kv[vert_spring_idx] = vert_spring_stiffnesses
    Kphi[tors_spring_idx] = tors_spring_stiffnesses

    # Assemble full stiffness matrix
    K = zeros(Float64, num_dof, num_dof)
    for ii in 1:num_elem
        nodes_ii = elem_nodes[ii, :]
        exy = nodes[nodes_ii, 1:2]

        Ke_g = bernoulli_beam_2d_torsion(exy[:, 1], exy[:, 2], elem_EI[ii], elem_GJ[ii], hinges[ii])

        # Start and end node dofs
        dofs_ij = [(3 * nodes_ii[1] - 2):(3 * nodes_ii[1]); (3 * nodes_ii[2] - 2):(3 * nodes_ii[2]);]

        # Add stiffnesses to global stiffness matrix
        K[dofs_ij[:], dofs_ij[:]] += Ke_g
    end

    link_element_dofs = zeros(Int, num_link_elem, 4)
    for ii in 1:num_link_elem
        nodes_ii = link_nodes[ii, :]
        exy = nodes[nodes_ii, 1:end]

        # `Ke_c` denotes the stiffness of the combined rigid link
        # and spring elements connecting the two girders referred
        # to the global degrees of freedom.
        Ke_c = spring_element_4dof(exy[:, 2], Kv[ii], Kphi[ii])

        # Start and end node dofs
        dofs_ij = [(3 * nodes_ii[1] - 2):2:(3 * nodes_ii[1]); (3 * nodes_ii[2] - 2):2:(3 * nodes_ii[2]);]

        # Add stiffnesses to global stiffness matrix
        K[dofs_ij[:], dofs_ij[:]] += Ke_c
        link_element_dofs[ii, :] = dofs_ij[:]
    end

    # Apply boundary conditions
    Ks_t, idx_keep = _apply_bcs_torsion(K, nodes, support_positions, support_dofs)

    return Ks_t, nodes, idx_keep, link_element_dofs
end
...
"""
Asssemble the global stiffness matrix for a twin girder model 
with torsion. The node numbering is as follows:
...
|---------->x
"""
function fem_general_single_girder_torsion(
    node_xs::AbstractArray{Float64,1},
    elem_EI::AbstractArray{Float64,1},
    elem_GJ::AbstractArray{Float64,1},
    support_positions::AbstractArray{Float64,1};
    support_dofs::AbstractArray{Int64}=zeros(Int, 0, 3),
    hinge_positions::AbstractArray{Float64,1}=zeros(0),
)
    # ................................................
    # Initialize
    ...
    support_dofs = _fem_general_input_check_torsion(
        node_xs, elem_EI, elem_GJ, support_positions, support_dofs
    )
    ...
    # Assemble the stiffness mx and apply bcs
    # ................................................
    K = _assemble_K_torsion(nodes, elem_nodes, elem_EI, elem_GJ, hinges)
    Ks, idx_keep = _apply_bcs_torsion(K, nodes, support_positions, support_dofs)
...
function _fem_general_input_check_torsion(
    node_xs,
    elem_EI,
    elem_GJ,
    support_positions,
    ...
    if length(elem_EI) != length(elem_GJ)
        throw(ArgumentError(
            "The length of `elem_EI` and elem_GJ should be equal"
        )
    )
end
    ...
    if length(support_dofs) == 0
        support_dofs = repeat([1, 0, 0]', length(support_positions))
    end
...
"""
    _assemble_K_torsion(nodes, elem_nodes, elem_EI, hinges) -> K
...
- `elem_nodes`: element connectivity matrix (`(N-1)×2`), each row is an element and the
    columns are: first node number; second node number (in correspondence with `nodes`).
- `elem_EI`: element flexural stiffness vector (`N-1`).
- `elem_GJ`: element torsional stiffness vector (`N-1`)
- `hinges`: vector indicating if an element has a hinge at one of its ends or not (`N-1`).

# Returns

- `K`: global stiffness matrix (`(3*N)×(3*N)`).

"""
function _assemble_K_torsion(nodes, elem_nodes, elem_EI, elem_GJ, hinges)
    # ................................................
    # Initialize
    # ................................................
    num_node = size(nodes, 1)
    ...
    for ii in 1:num_elem
        exy = nodes[elem_nodes[ii, :], 1:2]
        Ke = bernoulli_beam_2d_torsion(exy[:, 1], exy[:, 2], elem_EI[ii], elem_GJ[ii], hinges[ii])

        elem_dof_idx = (3 * ii - 2):(3 * ii + 3)
        K[elem_dof_idx, elem_dof_idx] += Ke
    end
...
"""
    _apply_bcs_torsion(K, nodes, support_positions, support_dofs) -> (Ks, idx_keep)
...
all.
"""
function _apply_bcs_torsion(K, nodes, support_positions, support_dofs)
    # Get the indices (of rows and columns) to be removed (fully constrained dofs)
    num_support = length(support_positions)
    idx_remove = []
    for ii in 1:num_support
        support_dof = support_dofs[ii, :]
        if any(support_dof .!= 0)
            idxs = 3 * findall(nodes[:, 1] .≈ support_positions[ii]) .- 2
            if support_dof[1] == 1
                append!(idx_remove, idxs)
            end
            if support_dof[2] == 1
...
nodes (`node_pairs`) connected by the springs. Translational (`type="trans"`), rotational
(`type="rot"`) and torsional (`type="tors"`) springs are available.

NOTE: It is assumed that dofs are ordered as: "trans", "rot", "tors" in the 
global stiffness matrix.
"""
function _linear_index_spring(node_pairs, num_global_dof; type, ndof=2)
    if type == "trans"
        offset = 1 - ndof
    elseif type == "rot"
        offset = 2 - ndof
